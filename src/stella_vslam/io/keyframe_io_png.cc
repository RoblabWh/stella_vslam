...
bool keyframe_io_png::save(const std::string& path,
    ...
    for (auto keyfrm : keyfrms)
    {
        ...
        {
            cv::imwrite(path + std::string("image") + std::to_string(keyfrm->id_) + std::string(".png"), keyfrm->img_);
        }
        if (!keyfrm->depth_.empty())
        {
            ...
            cv::imwrite(path + std::string("depth") + std::to_string(keyfrm->id_) + std::string(".png"), depth);
        }
        if (!keyfrm->mask_.empty())
        {
            cv::imwrite(path + std::string("mask") + std::to_string(keyfrm->id_) + std::string(".png"), keyfrm->mask_);
        }
    }
